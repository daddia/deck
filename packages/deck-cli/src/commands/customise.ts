...
export async function customise(resolver: Resolver, config: Config) {
  intro(picocolors.bgBlack(picocolors.whiteBright('Customise deck UI')));
    ...
    log.info(
      [
        ...
        picocolors.greenBright(
          '`deck-ui/layouts/docs` -> `@/components/layouts/docs`',
        ),
        pageAdded
          ? picocolors.greenBright(
              '`deck-ui/page` -> `@/components/layouts/page`',
            )
          : '',
      ].join('\n'),
    );
  ...
  if (result.target === 'home') {
    ...
    log.info(
      [
        ...
        'Open your `layout.tsx` files, replace the imports of components:',
        picocolors.greenBright(
          '`deck-ui/layouts/home` -> `@/components/layouts/home`',
        ),
      ].join('\n'),
    );
  }
