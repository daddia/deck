...
**deck is very flexible.** You can integrate with any content source, even without an official adapter.

> This guide assumes you are experienced with Next.js App Router.

### Examples

You can see examples to use deck with a CMS, which allows a nice experience on publishing content, and real-time update without re-building the app.

- [BaseHub](https://github.com/daddia/deck-basehub)
- [Sanity](https://github.com/daddia/deck-sanity)
...
```tsx
import { DocsPage, DocsBody } from 'deck-ui/page';
...
deck offers a simple [Algolia Search Adapter](/docs/headless/search/algolia), which includes a search client to integrate with deck UI.

## MDX Remote

deck offers the **MDX Remote** package, it is a helper to integrate Markdown-based content sources with deck.
You can think it as a `next-mdx-remote` with built-in plugins for deck.
...
```tsx
import { compileMDX } from '@deck/mdx-remote';
import { getPage } from './my-content-source';
import { DocsBody, DocsPage } from 'deck-ui/page';
