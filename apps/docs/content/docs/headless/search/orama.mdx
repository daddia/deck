...
description: Built-in document search of deck
---

deck supports searching document based on Orama.

As the built-in search of deck, It is the default but also recommended
...
Usually, it has been provided by your content source (e.g. deck MDX).
...
- **deck UI**: The built-in [Search UI](/docs/ui/search) supports it out-of-the-box.
- **Search Client**:
  ...
  <AutoTypeTable type='Extract<import("deck-core/search/client").Client, { type: "fetch" }>' />
...
<Callout type="info" title="For deck UI">
  You can ignore this, deck UI handles this when you have i18n configured
  correctly.
</Callout>
...
- **deck UI**: See [Static Export](/docs/ui/static-export#built-in-search) guide.
...
- **Search Client**:
  ...
  <AutoTypeTable type='Extract<import("deck-core/search/client").Client, { type: "static" }>' />
...
```ts
import { initAdvancedSearch } from 'deck-core/search/server';
