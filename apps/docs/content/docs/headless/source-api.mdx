...
`loader()` provides an interface for deck to integrate with file-system based content sources.
...
You can use it with built-in content sources like deck MDX.
...
```ts
import { Source } from 'deck-core/source';
