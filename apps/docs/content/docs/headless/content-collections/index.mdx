...
description: Use Content Collections for deck
---
...
```package-install
@deck/content-collections @content-collections/core @content-collections/mdx @content-collections/next
```
...
To integrate with deck, add the following to your `content-collections.ts`.
...
import { transformMDX } from '@deck/content-collections/configuration';
...
To use components from other packages like deck UI, pass them to your `<MDXContent />` component.
...
<Callout title='Deep Dive: Why?'>
    ...
    To support importing a package from node modules, deck added a default value to the `cwd` option of MDX Bundler.
