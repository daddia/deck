---
title: Themes
description: Add Theme to deck UI
---

## Usage

Only Tailwind CSS v4 is supported, the preset will also include source to deck UI itself:
...
deck supports light/dark modes with [`next-themes`](https://github.com/pacocoursey/next-themes), it is included in Root Provider.
...
deck UI adds its own colors, animations, and utilities with Tailwind CSS preset.
...
> The plugin works with and only with deck UI's MDX components, it may conflict with `@tailwindcss/typography`.
