---
title: Quick Start
description: Getting Started with deck
icon: Album
---

## Introduction

deck <span className='text-fd-muted-foreground text-sm'>(Foo-ma docs)</span> is a **documentation framework** based on Next.js, designed to be fast, flexible,
and composes seamlessly into Next.js App Router.

deck has different parts:

<Cards>

<Card icon={<CpuIcon className="text-purple-300" />} title='deck Core'>

Handles most of the logic, including document search, content source adapters, and Markdown extensions.

</Card>

<Card icon={<PanelsTopLeft className="text-blue-300" />} title='deck UI'>

The default theme of deck offers a beautiful look for documentation sites and interactive components.

</Card>

<Card icon={<Database />} title='Content Source'>

The source of your content, can be a CMS or local data layers like [deck MDX](/docs/mdx) (the official content source).

</Card>

<Card icon={<Terminal />} title='deck CLI'>
...
<Callout title="Want to learn more?">
  Read our in-depth [What is deck](/docs/ui/what-is-deck) introduction.
</Callout>

### Terminology

**Markdown/MDX:** Markdown is a markup language for creating formatted text. deck supports Markdown and MDX (superset of Markdown) out-of-the-box.
...
A new deck app should be initialized. Now you can start hacking!
...
<Cards>
  <Card href="/docs/ui/markdown" title="Markdown">
    deck has some additional features for authoring content.
  </Card>
...
<Tabs items={['deck MDX', 'Custom Source']}>
    ...
    <Tab value='Custom Source'>
        ...
        deck is not Markdown-exclusive. For other sources like Sanity, you can build a [custom content source](/docs/headless/custom-source).
...
Since deck uses Next.js App Router, you can simply rename the route:
...
And tell deck to use the new route in `source.ts`:
...
If you find anything confusing, please give your feedback on [Github Discussion](https://github.com/daddia/deck/discussions)!
...
<Cards>
  ...
  <Card
    href="/docs/ui/theme"
    title="Theming"
    description="Add themes to deck UI"
  />
  <Card
