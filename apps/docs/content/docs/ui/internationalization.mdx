...
<Callout title='Before you get started'>
    ...
    deck is not a full-powered i18n library, it manages only its own components and utilities.
...
And update deck UI layout options.
...
import type { Translations } from 'deck-ui/i18n';
...
Pass the locale to deck in your pages and layouts.
...
deck only handles navigation for its own layouts (e.g. sidebar).
...
In addition, the [`deck-core/dynamic-link`](/docs/headless/components/link#dynamic-hrefs) component supports dynamic hrefs, you can use it to attend the locale prefix.
It is useful for Markdown/MDX content.

```mdx title="content.mdx"
import { DynamicLink } from 'deck-core/dynamic-link';
