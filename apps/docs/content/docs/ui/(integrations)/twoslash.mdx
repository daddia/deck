...
```ts twoslash title="source.config.ts (deck MDX)"
import { defineConfig } from 'deck-mdx/config';
import { transformerTwoslash } from 'deck-twoslash';
import { rehypeCodeDefaultOptions } from 'deck-core/mdx-plugins';
...
```ts twoslash
import { generateFiles } from 'deck-openapi';
