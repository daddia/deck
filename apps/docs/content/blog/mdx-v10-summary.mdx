...
```ts
import createMDX from 'deck-mdx/config';
...
Now you can generate types using `deck-mdx` command, it's recommended to set it as a `postinstall` script.
...
Now to integrate it with deck framework, change your `source.ts` from:
...
You can read the latest docs of deck MDX for details.

<Cards>
  <Card href="/docs/mdx" title="deck MDX">
