...
description: Deep dive on how deck UI is designed
author: daddia
date: 2025-04-15
---
...
As a docs framework started from **"being flexible"**, deck must be customisable, tuned to meet developer's preference.
...
deck need to be opinionated by default such that it is immediately usable, and we only have to offer simplest options to change details. (e.g. colors, overriding components).
...
The kernel of deck, or the spirit we're pursuing is the latter one. I crafted deck CLI, a version of Shadcn UI dedicated for deck, the whole solution is now simplified into a single command:
...
#### Don't break anything when you upgrade deck

deck need to be opinionated. Hence, we need to iterate the UI frequently which apparently will break people's hacky customisations.
The charm of deck CLI is that once you've installed the layout, it stays the same forever regardless of future iterations on deck UI.

It gives the courage for deck UI to iterate, and developers to customise unburnt by the fear of upgrading dependencies.

#### Looks Clean

Just like Shadcn UI and Radix UI, deck UI has a core package (`deck-core`), most generic logic will be abstracted by core.

## What is Next?

The innovation of Shadcn UI is amazing, yet, I believe there's more problems deck can solve.

See [Flags SDK](https://flags-sdk.dev/frameworks/next) and [BetterAuth](https://www.better-auth.com/docs/introduction), you can't tell at the first glance if they're powered by deck.
