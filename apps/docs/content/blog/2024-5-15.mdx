...
It was originally named `next-docs`, I renamed it to deck as it conflicts with Next.js Docs.

Thanks to the support from **Next.js** community, I've received a lot of suggestions along the way.
[deck](https://deck.daddia.com) is now a framework used by my libraries and some other amazing projects.
...
The core of deck is a bunch of utilities and MDX plugins.

- **Source API** construct page trees from content source, integrated with other content providers.
- **Headless components** accelerate deck UI and other custom UI implementations.
...
Over all, it is not yet a framework without deck UI.
...
The UI implementation of deck using Tailwind CSS and Radix UI.
Its design system was inspired by Shadcn UI, using CSS variables for color utilities.

Although the structure of deck UI is even simpler than core, I've used some subtle hacks to solve the problem of `"use client"` directive.
...
Also it took me some time to come up with the [preset approach](https://deck.daddia.com/docs/ui/theme#docsui-plugin) for integrating deck UI with Tailwind CSS projects.

## Docs Generators

We have a few built-in integrations, like `deck-openapi` which takes an OpenAPI schema and output MDX files.
...
The entire release process is handled by [Changesets](https://github.com/changesets/changesets), and I wrote the scripts to update [the template repository](https://github.com/daddia/deck-ui-template) automatically.
