...
deck v15 has no other significant changes than migrating the config to Tailwind CSS v4.
...
Add `@import` to the presets exported by deck UI, and include `deck-ui` package into source.
...
deck no longer use `--fd-<color>` like `--fd-primary: 0 0% 0%` for colors, it directly defines and uses colors in `@theme`.
...
v15 improved the playground UI (inspired by the minimalism of Scalar), and brought Scalar API Client support to deck OpenAPI.

You can enable the Scalar API Client using `useScalar` option in `createOpenAPI()`:

```ts
import { createOpenAPI } from 'deck-openapi/server';
import { APIPlayground } from 'deck-openapi/scalar';
...
<Callout>
  ...
  stylesheet of deck UI will conflict with their `style.css` because both
...
In the future, we also want to make further improvements to deck MDX:
