---
title: deck v14
description: Introducing deck v14
author: Fuma Nama
date: 2024-09-19
---
...
Taking some inspirations from [Shadcn UI](https://ui.shadcn.com), I decided to make deck easier to use, with more APIs that simplify the design.
...
Clone deck UI components to your workspace and modify them.

```package-install
npm install @deck/cli --save-dev
```
...
Previously, multiple page tree is implemented with deck UI `RootToggle` component.
You have to add it to the sidebar banner which isn't as intuitive as other APIs.

With Sidebar Tabs, by creating a root folder, deck will immediately add a tabs component to the sidebar.
...
In favour of this, the new search client of deck is introduced with support for different API clients.
...
I'm always working to reduce the dependencies required for deck, that is one reason why we separated deck into different packages.

deck UI now bundles icons from `lucide-react`, for Next.js apps that is not using Lucide, they can avoid downloading the entire icon library.
...
To improve code organization, we made a Metadata Image API on deck Core.
...
In favour of this, the `getImageMeta` function from `deck-ui/og` has been removed.

### Shorthands

As you may notice, we introduced more abstractions to reduce the setup steps required for enabling some features.
It is also required for the code generator from deck CLI to work.
...
deck UI Card can now support usage without `href`.
You can pass children as React node, Typography styles will be applied correctly.

```mdx
<Card icon={<Database />} title='Content Source'>

The input/source of your content, it can be a CMS, or local data layers like [Content Collections](https://www.content-collections.dev) and [deck MDX](/docs/mdx).
...
```tsx title="page.tsx"
import { source } from '@/lib/source';
import { DocsCategory } from 'deck-ui/page';
...
deck v14 is compatible with Next.js 15, supporting sync and async usages of dynamic APIs.
...
```ts
import { DocsLayout } from 'deck-ui/layout';
import { HomeLayout } from 'deck-ui/home-layout';
import { HomeLayoutProps } from 'deck-ui/home-layout';
```
...
We moved Twoslash UI components to `deck-twoslash`.
This isolates some logic from deck UI, allowing a better code organization.
...
The types from `deck-core/search/shared` is moved to `deck-core/server`.
